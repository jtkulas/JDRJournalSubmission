---
title             : "Demanding resources: Similarity of perceptions for challenge and resource characteristics"
...
We used `r cite_r("r-references.bib")` for all analyses. Our analyses are presented by characteristics of work that are rated as being "relevant". Upon confirming that a work characteristic was present, respondents then also rated the extent to which that characteristic was percieved as a resources, challenge, and hindrance. 
...
Figure \@ref(fig:percagree) shows that there was not much mutual agreement regarding characteristics viewed as both hindrances and resources ($\bar{X}$ = `r round(mean(rh$mean, na.rm=TRUE),2)`) or as challenges and hindrances 
($\bar{X}$ = `r round(mean(ch$mean, na.rm=TRUE),2)`). Alternatively, whether a characteristic was viewed as both a resource and a challenge exhibited greater consistency although also had quite a bit of variability ($\bar{X}$ = `r round(mean(rc$mean, na.rm=TRUE),2)`, *sd* = `r round(sd(rc$mean, na.rm=TRUE),2)`).
...
F ratio from a repeated measures ANOVA is $F_(6, 3,402)$ = 613.5, *p* < .001 (the subjects' effect was $F_{(567, 3402)}$ = 6.13, *p* < .001.
