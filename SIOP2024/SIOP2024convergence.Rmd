...
#levenes <- leveneTest(value ~ scale, data=analysis)
...
Figure \@ref(fig:recchall) explores the possibility of moderation by *type of characteristic rated* for the resource-challenge convergence. Here we categorized each characteristic by its O\*NET "scale" (one of seven), and the graph shows greater consistency across certain characteristics (for example, *Mental Processes* or *Interacting with Others*) and less convergence across other *types* of job activities (for example, *Physical* characteristics). A repeated-measures ANOVA retaining these scales as 7 different levels of a within-subjects' independent variables yielded a treatment effect of $F_{(6, 3,402)}$ = 613.5, *p* < .001, $\eta^2_{p}$ = `r round(repmeas$ANOVA[2,9],2)` (the subjects' effect was $F_{(567, 3402)}$ = 6.13, *p* < .001. The only contrast approaching statistical significance via Tukey's HSD specification (*p* = .09) was the contrast between Physical Labor (*M* = 0.49) and Business and Financial Operations (*M* = 0.58). 
