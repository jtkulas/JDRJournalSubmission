...
Figure \@ref(fig:recchall) explores the possibility of moderation by *type of characteristic rated* for the resource-challenge convergence. Here we categorized each characteristic by its O\*NET "scale" (one of seven), and the graph shows greater consistency across certain characteristics (for example, *Mental Processes* or *Interacting with Others*) and less convergence across other *types* of job activities (for example, *Physical* characteristics). A repeated-measures ANOVA retaining these scales as 7 different levels of a within-subjects' independent variables yielded a treatment effect of $F_{(6, 3,402)}$ = 613.5, *p* < .001, $\eta^2_{p}$ = `r round(repmeas$ANOVA[2,9],2)` (the subjects' effect was $F_{(567, 3402)}$ = 6.13, *p* < .001. The only contrasts not broaching statistical significance via Bonferonni-corrected pairwise t-tests were Work Output vs. Physical Work Conditions (*t* = -2.31), Interpersonal Relationships vs. Mental Processes (*t* = .18), and Information Input vs. Physical Work Conditions (*t* = 1.64). The largest contrasts were Interacting with Others vs. Work Output (*t* = 36.09) and Information Input vs. Interacting with Others (*t* = -34.38) and Mental Processes (*t* = -34.10) 
