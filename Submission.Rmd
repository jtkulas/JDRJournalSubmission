...
rstatix::anova_test(data=overwork,dv=score,wid=subject,within=type) 

## trying to replicate - doesn't work 1/31/23

trying <- aov(formula = score ~ type + Error(subject/type), data=overwork)
summary(trying)

## pairwise_t_test( value~time,paired=TRUE, p.adjust.method = "bonferroni" ) 

# check <- overwork[,c(406:407)]
# psych::describe.by(check, group=check$type)
