...
abstract: |
 **Purpose**: Discussions of job characteristics often include a tacit characterization that these job elements are either resources or demands. The purpose of this paper was to examine 1) variability in subjective ratings of job characteristics with respect to perception as resource and/or demand, 2) differential association with positive and negative outcomes, and 3) the potential buffering effect of resources on the relationships between demands and negative work outcomes. **Design/methodology/approach**: A sample of 568 respondents recruited from Prolific completed an online survey. They were asked to think only about their primary job, and each administered survey branched content dependent on whether this job could be described as consisting of the different O\*NET job characteristics. They subsequently rated level of resource, challenge, or hindrance. **Findings**: Results support the perspective that job characteristics are not uniquely categorized as a resource or demand, but rather, what is viewed as challenging is also commonly interpreted as resourceful. Furthermore, greater experienced resources moderate the challenge–engagement relationship as well as the hindrance-stress and hindrance–burnout relationships. **Originality/value**: We utilized a diverse sample of employees across industries who responded to items derived from O*NET specifications. O*NET provides a rich and fairly exhaustive repository of job characteristics that also provides some consistency across investigations. We also explored characteristics at a higher level of aggregation (retaining O*NET category groupings), which again expands our collective knowledge of how employees perceive different categories of resources, challenges, and hindrances within different work domains, as well as the relationships among them. 
...
The Occupational Information Network (O\*NET) serves as a repository of information broadly descriptive of American occupations [@peterson2001understanding] and these descriptions are frequently updated. We focused on 98 work [activity and context statements](https://www.ONETonline.org/find/descriptor/result/4.A.1.b.3) which O\*NET groups into *activity* categories of information input (e.g., where and how is the information and data gained that is needed to perform the job?), interacting with others (e.g., what interactions with other persons or supervisory activities occur while performing the job?), mental processes (e.g., what processing, planning, problem-solving, decision-making, and innovating activities are performed with job-relevant information?) and work output (e.g., what physical activities are performed, what equipment and vehicles are operated/controlled, and what complex/technical activities are accomplished as job outputs?). Work *context* statements are grouped into interpersonal relationships (e.g., the context of the job in terms of human interaction processes), physical work conditions (e.g., the work context as it relates to the interactions between the worker and the physical job environment), and structural job characteristics (e.g., the interactions between the worker and the structural characteristics of the job). 

O\*NET collects information about these elements by periodically surveying workers with job characteristic questions, each of which often has [unique response categories](https://www.ONETonline.org/find/descriptor/result/4.C.1.c.2).  For example, "How responsible is the worker for work outcomes and results of other workers?" has response options ranging from *no responsibility* to *very high responsibility*, while the question, "How often do you use electronic mail in this job?" has options ranging from *never* to *every day*. All response scales, regardless of semantic labeling, have the same *number* of response option categories (five). For the current investigation, O\*NET’s uniquely--labeled response scales were retained when asking for *statement relevance*. We also retained the original O\*NET wording for our item stems, with minor grammatical referent exceptions^[For example, the worker responsibility question was changed from "How responsible is the worker..." to "How responsible are you..."]. The total number of items on each administered survey was less than 392 (98 characteristics x 4 repeated measurements) because we did not ask for evaluations for 14 O\*NET characteristics that we surmised to have extremely low frequency of affirmative endorsement across respondents (one such excluded characteristic, for example, was "...the extent to which the worker is exposed to radiation on the job"). 
...
Stress was captured via three items taken from the Copenhagen Psychosocial Questionnaire [@burr2019third]. Responses were provided using a Likert-type scale ranging from (1) Not at All to (5) All the Time. A sample item is, “How often have you had problems relaxing because of your job?”. Coefficient alpha [@cronbach1951coefficient] was 0.85 in this sample. 

### Burnout 

Four items from the Copenhagen Psychosocial Questionnaire [@burr2019third] were used to assess burnout. Again, responses were provided using a Likert-type scale ranging from (1) Not at All to (5) All the Time. A sample item is, “How often have you felt worn out?”. Within our sample, coefficient alpha was 0.85.
...
```{r overallhist, fig.cap="Frequency distributions of standard deviations across characteristics rated as resources, challenges, and demands."}

## Moved up from lower location 9/30 because of Figure numbering within paper
## resource$sd
...
library(gridExtra)
grid.arrange(e, d, f, nrow = 3)

```

H1 states that *not everyone agrees* that a given characterisitic is a work hindrance, challenge, or resource. Figure \@ref(fig:overallhist) presents standard deviations of characteristic rating across all rated items. Here, the *spread* of difference exhibits wider distributions of disagreement for challenge and resource ratings (and a relatively more leptokurtic and elevated distribution of disagreement for hindrances). These distributions reflect that some characteristics are largely agreed upon as being challenges and resources, while all hindrance perceptions exhibit a relatively higher level of disagreement. A Bartlett's test for homogeneity of variance across the challenge, hindrance, and resource ratings confirms this difference ($\chi^2_{`r lookie$df`}$ = `r lookie$statistic[[1]]`, *p* < .01). This points to *hindrances*, in particular, as being differentially experienced across workers (whereas characteristics exist that are agreed--upon as being (un)resourceful or (non)challenging). 

For some descriptive context, Figures \@ref(fig:combinegraphs) and \@ref(fig:combinegraphs2) present the most extreme examples of characteristic variability in the form of the top eight *similarly rated* (Figure \@ref(fig:combinegraphs)) and *dissimilarly rated* (Figure \@ref(fig:combinegraphs2)) resources, challenges, and demands.[^onlinelist] These figures again highlight that what is perceived as resource and challenge can, with some characteristics, be agreed upon (the range of the "lowest 8" resource standard deviations is `r min(topresource$sd)` to `r max(topresource$sd)` and the range of lowest 8 challenge standard deviations is `r min(topchallenge$sd)` to `r max(topchallenge$sd)`). However, there is considerably less agreement regarding the degree to which job elements should be considered to be hindrances, with the 8 elements showing the *greatest agreement* still ranging in fairly large standard deviations (ranging from `r min(tophindrance$sd)` to `r max(tophindrance$sd)`). In sum, these results provide support for H1, and particularly so for hindrances.

[^onlinelist]: A full list of item characteristic ratings, along with summary averages and standard deviations is available in supplementary online resources. The Figure \@ref(fig:combinegraphs) and \@ref(fig:combinegraphs2) presentations are limited to "top eight" characteristics per category in consideration of page--space (there are 252 individual characteristic ratings in the online resources).

The second hypothesis stated that job characteristics would not be *exclusively* categorized as a resource or demand. We explore this via pattern of correlation, procedurally derivative of @campbell1959convergent's multi--trait multi--method matrix exploration. Table \@ref(tab:cortab) provides correlations among O\*NET item "scale"--level groupings^[Survey items are grouped by O\*NET into categories of Information Input (ii), Mental Processes (mp), Work Output (wo), Interacting with Others (io), Interpersonal Relationships (ir), Physical Work Conditions (pc), and Structural Job Characteristics (sc). These groupings were not presented to respondents but are leveraged here to facilitate interpretation of characteristic association.] across resource, challenge, and hindrance evaluations. Here one would expect only trivial associations *across* resource, challenge, and hindrance perceptions if job characteristics were uniquely categorized as such. 

First, the average correlation within all resource categories (variables 1 through 7 in Table 1) was .43 (*SD* = .13, coefficient magnitudes range from .15 to .64), and challenge categories exhibited similar associations (ranging from .12 to .70, *M* = .43, *SD* =.16). Job hindrances, however, exhibited relatively elevated associations across categories, with correlations ranging from .33 to .86, *M* = .62, *SD* = .17. When people perceive hindrances, these seem to be more likely to be shared across different types of job activities, whereas challenges and resources can be relatively isolated within different job activities. 

Regarding comparisons *across perception type*, the mean resource to challenge correlations within the same O\*Net grouping ranged from .62 to .66 (*M* = .64, *SD* = .02; for example, the association between information input ratings as a resource and information input challenges was .62). The correlations between resources and challenges *across* O\*Net grouping ranged from .08 to .50, *M* = .32, *SD* = .12 (for example, the correlation between mental processes [resource] and work output [challenge] was .39). The resource-hindrance correlations within the same O\*Net grouping ranged from -.16 to -.30 (*M* = -.24, *SD* = .05), while the correlations between resources and hindrances *across* O\*Net grouping ranged from .05 to -.27 (*M* = -.14, *SD* = .08). The mean challenge to hindrance correlations within the same O\*Net grouping ranged from -.04 to -.27 (*M* = -.21, *SD* = .08). The correlations between challenges to hindrances across O\*Net grouping ranged from .12 to -.26, *M* = -.11, *SD* = .09.  In summary, observed correlations were larger  *within* O\*Net grouping than *across* (for all comparisons). Challenge and resource demands demonstrated the largest associations, with even the magnitude of across--category associations (.32) exceeding all hindrance perception associations -.11 to -.24). These results provide support for H2 -- particularly regarding *resource* and *challenge* perceptions. Stated differently, job characteristics should not be expected to be uniquely categorized as resource or demand.
...
papaja::apa_table(r_second$apa, 
                  caption = "Outcome variable bivariate correlations.",
                  escape = FALSE, 
...
Table 2 presents empirical  associations between estimates of resource, challenge, and hindrance and worker *engagement*, *stress*, and *burnout*. Here, workers' perceptions were aggregated across all characteristics to provide indications of "overall" resource and demand experience. Engagement exhibited non-trivial associations with all three types of perception whereas stress was only associated with the hindrance aggregate. The more percieved hindrances, the higher the rating of stress ($r$=.11, $p$<.01), indicating only weak partial support for Hypothesis 3b. All subsequent analyses retain this "overall" operationalization of workers' experience of resources, challenges, and hindrances. 

### Challenges, Resources, and Job Outcomes

Hypotheses 3a, 4a, and 4b were formally tested via targeted hierarchical regressions. H3a predicted that both resources and challenges would predict engagement. Table \@ref(tab:chal-resource-table) summarizes the results for the hypothesized engagement effect as well as the H4a investigations of both stress and burnout. As expected given the Table 2 bivariate associations, the combined effect of challenges and resources did explain a statistically significant amount of the variability in engagement, $R^2$ = `r round(summary(cr_eng_m2)$r.squared, 2)`, Adj. $R^2$ =  `r round(summary(cr_eng_m2)$adj.r.squared, 2)`, `r cr_eng_info2$statistic$modelfit$r2`.  However, the resource slope is significant, wheras the challenge slope is not significant. This results in only partial support for H3a. The inclusion of the interaction term in step two of the model was not hypothesized but is included for completeness. The interaction effect did contribute a significant addition to the model, `r cr_eng_info3$statistic$modelfit$r2`, $\Delta R^2$ = `r cr_eng_r_square_change`, $\Delta F$ (`r cr_eng_anova$Df[2]`, `r cr_eng_anova$Res.Df[2]`) = `r cr_eng_anova$F[2]`, and thus provides some statistical support for the possibility of moderation. Figure \@ref(fig:chal-resource-int) illustrates the nature of the interaction. With low levels of resources, the relationship between challenges and engagement is relatively flat and engagement is comparatively low. With more resources, the relationship between challenges and engagement is negative, but engagement still remains higher with greater reported challenge when more resources are perceived.

Regarding Hypothesis 4a, challenge demands and resources did not explain a significant amount of the variance in stress,  $R^2$ = `r round(summary(cr_stress_m2)$r.squared, 2)`, Adj. $R^2$ =  0.00, `r cr_stress_info2$statistic$modelfit$r2`. The inclusion of the interaction term in step two of the model did not contribute a significant addition to the model, `r cr_stress_info3$statistic$modelfit$r2`, $\Delta R^2$ = `r cr_stress_r_square_change`, $\Delta F$ (`r cr_stress_anova$Df[2]`, `r cr_stress_anova$Res.Df[2]`) = `r cr_stress_anova$F[2]`, and thus does not support the presence of moderation. These challenge demands and resources did, however, explain a statistically significant amount of the variability in burnout, $R^2$ = `r round(summary(cr_burn_m2)$r.squared, 2)`, Adj. $R^2$ =  `r round(summary(cr_burn_m2)$adj.r.squared, 2)`, `r cr_burn_info2$statistic$modelfit$r2`. The inclusion of the interaction term in step two of the model did not contribute a significant addition to the model, `r cr_burn_info3$statistic$modelfit$r2`, $\Delta R^2$ = `r cr_burn_r_square_change`, $\Delta F$ (`r cr_burn_anova$Df[2]`, `r cr_burn_anova$Res.Df[2]`) = `r cr_burn_anova$F[2]`, and thus collectively these analyses fail to provide statistical support for the presence of moderation (Hypothesis 4a). In sum, these findings do not provide support for the assertion that resources would moderate the relationships between challenge demands and the outcomes of strain and burnout.
...
The major aims of this paper were to explore whether: 1) there was variability in subjective ratings of job characteristics as resources and demands, 2) subjective appraisals were differentially related to positive and negative outcomes, and lastly, 3) if resources buffer the relationships between demands (challenge and hindrance) and outcomes. We found that job characteristics were not uniquely categorized as a resource or demand, but rather, job characteristics were often viewed similarly as both a resource and a challenging demand. The findings broadly revealed further that there was relatively more consistency in ratings of resource and challenge characteristics, and far more variability in hindering elements across workers and jobs. While engagement exhibits moderate associations with all investigated perceptions, we did not find support for the prediction that different types of demands would be related to stress and burnout, but we did find that resources moderated the challenge-engagement relationship, and further, resources moderated the hindrance-stress, and hindrance-burnout relationships (as predicted). 

In addition to the above findings, this paper makes several additional important contributions. We utilized a diverse sample of employees across industries who responded to items derived from O\*NET specifications. O\*NET provides a rich and fairly exhaustive repository of job characteristics that also provides some consistency across investigations -- a somewhat "standardized" lens through which jobs can be deconstructed and investigations compared. We also explored characteristics at a higher level of aggregation (retaining O\*NET category groupings), which again expands our collective knowledge of how employees perceive different categories of resources, challenges, and hindrances within different work domains, as well as the relationships among them.

Perceptions of resources and demands, broadly, are not universal -- there are individual differences in how employees experience job characteristics. This finding aligns quite well with the challenge-hindrance stressor framework, which posits that employees perceive stimuli (i.e., job characteristics) uniquely [@lazarus1984stress], and thus, could appraise them as either a challenge or hindrance to their job performance [@cavanaugh2000empirical]. Further, @cavanaugh2000empirical suggests that challenge demands are typically associated with positive outcomes and hindrance demands are associated with negative outcomes [e.g., @cavanaugh2000empirical], which we partially supported here. 
