...
H1 proposes that job characteristics differ in consistancy regarding subjective worker perception as a demand or resource. We explore this hypothesis first by item, and then present a more holistic picture. First considering item-level characteristics, we present means and standard deviations visually, focusing on the items highest and lowest in variability. Figure \@ref(fig:combinegraphs) presents the top 8 resources, challenges, and hindrances characteristics that are *largely agreed on* as indexed by (relatively) low standard deviations. Note that the presentation is limited to 8 characteristics per perceived category only because of limited space. All sortable ratings are available in the online resources.

Figures \@ref(fig:combinegraphs) and \@ref(fig:combinegraphs2) present only a snapshot - full list available in online resources. 

As can be seen in the figures, what is widely seen as a resource and challenge tends to be somewhat agreed upon (range of lowest 8 resource standard deviations is `r min(topresource$sd)` to `r max(topresource$sd)` and the range of lowest 8 challenge standard deviations is `r min(topchallenge$sd)` to `r max(topchallenge$sd)`. However, there is considerable disagreement regarding the degree to which job elements are considered hindrances, with the 8 elements showing the *greatest agreement* still ranging in standard deviations from `r min(tophindrance$sd)` to `r max(tophindrance$sd)`.

Next, we explore the items with the highest variability in perceptions as resources, challenges, and hindrances. Figure \@ref(fig:combinegraphs2) presents the resources, challenges, and hindrances that are *largely disagreed on* as indexed by (relatively) high standard deviations. Stated another way, these are the 8 characteristics with the greatest variability in rating. In sum, these results provide some support for H1, particularly for hindrances, which are consistently viewed as "different". 

In addition to highlighting specific items, we also look at the pattern more holistically. Figure \@ref(fig:overallhist) shows that hindrances tend to exhibit similar levels of variability across items. Challenges and resources, however, exhibit items that are rated more similary across people (note the spread of the challenge and resource histograms relative to the hindrance histogram). A Bartlett's test for homogeneity of variance confirms this difference ($\chi^2_{`r lookie$df`}$ = `r lookie$statistic[[1]]`, *p* = `r lookie$p.value` ). 
...
In addition to the two hypotheses, two related research questions were proposed. To answer these research questions, authors first categorized items into the JD-R elements listed in the JD-R literature. For example, autonomy is frequently described as a resource. An O\*Net item in the autonomy category is, “How much decision making freedom, without supervision, does your job offer?”. This O\*Net item was therefore retained within the "autonomy" category. Means for resource, challenge, and hindrance groupings were then computed to explore whether literature-implicated resources and demands were evaluated as such. The left side of Figure 4 shows literature-derived demand categories (e.g., work pressure). Here, we do not see a clear demarcation of resource and challenge, as would be expected if the job characteristics evidenced consistency (the literature-driven consistency would manifest as "high" gray and black bars and "low" white bars). In alignment with what we observed regarding variability in ratings of hindrance stressors in H1, there is much less consistency in how employees rated what should objectively be "hindrances" at work. However, in contrast, there is a clearer pattern of the highest level ratings being that of resources on the right side of Figure 4 showing literature-derived resources (e.g., job control) and the corresponding average category ratings. The white bars representing resources are consistently higher. In sum, these results provide some support for RQ 1 and 2. 
