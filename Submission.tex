...
\NormalTok{foranova }\OtherTok{\textless{}{-}}\NormalTok{ data}
\NormalTok{foranova}\SpecialCharTok{$}\NormalTok{subject }\OtherTok{\textless{}{-}} \DecValTok{1}\SpecialCharTok{:}\FunctionTok{nrow}\NormalTok{(foranova) }
  
\NormalTok{anova1 }\OtherTok{\textless{}{-}} \FunctionTok{gather}\NormalTok{(foranova,type,score,resources\_workpressure}\SpecialCharTok{:}\NormalTok{hindrance\_autonomy, }\AttributeTok{factor\_key=}\ConstantTok{TRUE}\NormalTok{)}

\DocumentationTok{\#\# now need to isolate rows with targeted characteristics}

\NormalTok{overwork }\OtherTok{\textless{}{-}}\NormalTok{ anova1[ }\FunctionTok{which}\NormalTok{(anova1}\SpecialCharTok{$}\NormalTok{type}\SpecialCharTok{==}\StringTok{\textquotesingle{}resources\_phyworkload\_wkover\textquotesingle{}} \SpecialCharTok{|}\NormalTok{ anova1}\SpecialCharTok{$}\NormalTok{type}\SpecialCharTok{==}\StringTok{\textquotesingle{}challenge\_phyworkload\_wkover\textquotesingle{}} \SpecialCharTok{|}\NormalTok{ anova1}\SpecialCharTok{$}\NormalTok{type}\SpecialCharTok{==}\StringTok{\textquotesingle{}hindrance\_phyworkload\_wkover\textquotesingle{}}\NormalTok{), ]}

\NormalTok{rstatix}\SpecialCharTok{::}\FunctionTok{anova\_test}\NormalTok{(}\AttributeTok{data=}\NormalTok{overwork,}\AttributeTok{dv=}\NormalTok{score,}\AttributeTok{wid=}\NormalTok{subject,}\AttributeTok{within=}\NormalTok{type) }
\end{Highlighting}
\end{Shaded}
...
## 1   type   2 1134 17.709 2.67e-08     * 0.017
## 
## $`Mauchly's Test for Sphericity`
##   Effect     W        p p<.05
## 1   type 0.694 1.29e-45     *
## 
## $`Sphericity Corrections`
##   Effect   GGe       DF[GG]    p[GG] p[GG]<.05   HFe      DF[HF]    p[HF]
## 1   type 0.766 1.53, 868.32 7.09e-07         * 0.767 1.53, 870.2 6.93e-07
##   p[HF]<.05
## 1         *
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{trying }\OtherTok{\textless{}{-}} \FunctionTok{aov}\NormalTok{(}\AttributeTok{formula =}\NormalTok{ score }\SpecialCharTok{\textasciitilde{}}\NormalTok{ type }\SpecialCharTok{+} \FunctionTok{Error}\NormalTok{(subject}\SpecialCharTok{/}\NormalTok{type), }\AttributeTok{data=}\NormalTok{overwork)}
\FunctionTok{summary}\NormalTok{(trying)}
\end{Highlighting}
\end{Shaded}
...
## Error: Within
##             Df Sum Sq Mean Sq F value  Pr(>F)   
## type         2    9.2   4.586   5.989 0.00256 **
## Residuals 1698 1300.2   0.766                   
## ---
...
\begin{Shaded}
\begin{Highlighting}[]
\DocumentationTok{\#\# pairwise\_t\_test( value\textasciitilde{}time,paired=TRUE, p.adjust.method = "bonferroni" ) }

\CommentTok{\# check \textless{}{-} overwork[,c(406:407)]}
\CommentTok{\# psych::describe.by(check, group=check$type)}
\end{Highlighting}
\end{Shaded}
