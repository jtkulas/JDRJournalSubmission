...
Similarly, hindrance stressors and resources explained XX\% of the variance in burnout, \(R^2\) = .xx. Adj. \(R^2\) = .xx, \emph{F}(x, xx) = xx.xx, \emph{p} xxx. The inclusion of the interaction term in step two of the model contributed a significant addition to the model, \(Delta R^2\) = .xx, \(Delta F\)(xx, xx) = .xx, \emph{p} = xx. The total model explained xx\% of the variance in burnout,\(R^2\) = .xx. Adj. \(R^2\) = .xx, \emph{F}(x, xx) = xx.xx, \emph{p} \textless.xxx. Again, see Figure \ref{fig:hind-resource-burn-int}. As expected, the relationship between hindrance stressors and burnout becomes weaker as workers perceive more resources.
