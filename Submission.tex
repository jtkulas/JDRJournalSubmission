...
The findings regarding demands are more complex. When a demand is appraised as a hindrance, it is negatively associated with motivational resources (Kim \& Beehr, 2020), engagement (Crawford et al., 2010), job search behaviors, and job satisfaction (Cavanaugh et al., 2000). Chen et al. (2021) further found that daily hindrance demands were negatively associated with cognitive wellbeing and work family enrichment. Turnover intentions, actual turnover and withdrawal behaviors have also been associated with hindrance demands (Podsakoff et al., 2007). Appraising a demand as a \emph{challenge}, however, has been positively associated with sources of motivation (i.e., sense of self-worth and meaningful work, Chen et al. (2021), engagement, Crawford et al. (2010), and strain and turnover intentions, e.g., Abbas and Raja (2019)). Challenge appraisals have also been found to ehave negative associations with job search behaviors (e.g., Cavanaugh et al., 2000). Interestingly, both challenges and hindrances have been shown to positively predict strain (Abbas \& Raja, 2019; J. A. LePine et al., 2005; Podsakoff et al., 2007; Webster et al., 2010), which further highlights the complex association between characteristic appraisals and important work outcomes. Given the differential relationships described above, and a focus on three outcomes (engagement, strain, and burnout) we make the following predictions (see Figure \ref{fig:ourmodel}):
...
H1 states that \emph{not everyone agrees} that a given characterisitic is a work hindrance, challenge, or resource. Figure \ref{fig:overallhist} presents standard deviations of characteristic rating across all rated items. Here, the \emph{spread} of difference exhibits wider distributions of disagreement for challenge and resource ratings (and a relatively more leptokurtic and elevated distribution of disagreement for hindrances). These distributions reflect that some characteristics are largely agreed upon as being challenges and resources, while all hindrance perceptions exhibit a relatively higher level of disagreement. A Bartlett's test for homogeneity of variance across the challenge, hindrance, and resource ratings confirms this difference (\(\chi^2_{}\) = 76.83, \emph{p} \textless{} .01). This points to \emph{hindrances}, in particular, as being differentially experienced across workers (whereas characteristics exist that are agreed--upon as being (un)resourceful or (non)challenging).

For some descriptive context, Figures \ref{fig:combinegraphs} and \ref{fig:combinegraphs2} present the most extreme examples of characteristic variability in the form of the top eight \emph{similarly rated} (Figure \ref{fig:combinegraphs}) and \emph{dissimilarly rated} (Figure \ref{fig:combinegraphs2}) resources, challenges, and demands.\footnote{A full list of item characteristic ratings, along with summary averages and standard deviations is available in supplementary online resources. The Figure \ref{fig:combinegraphs} and \ref{fig:combinegraphs2} presentations are limited to ``top eight'' characteristics per category in consideration of page--space (there are 252 individual characteristic ratings in the online resources).} These figures again highlight that what is perceived as resource and challenge can, with some characteristics, be agreed upon (the range of the ``lowest 8'' resource standard deviations is 0.70 to 0.88 and the range of lowest 8 challenge standard deviations is 0.79 to 0.86). However, there is considerably less agreement regarding the degree to which job elements should be considered to be hindrances, with the 8 elements showing the \emph{greatest agreement} still ranging in fairly large standard deviations (ranging from 1.12 to 1.16). In sum, these results provide support for H1, and particularly so for hindrances.

The second hypothesis stated that job characteristics would not be \emph{exclusively} categorized as a resource or demand. We explore this via pattern of correlation, procedurally derivative of Campbell and Fiske (1959)'s multi--trait multi--method matrix exploration. Table \ref{tab:cortab} provides correlations among O*NET item ``scale''--level groupings\footnote{Survey items are grouped by O*NET into categories of Information Input (ii), Mental Processes (mp), Work Output (wo), Interacting with Others (io), Interpersonal Relationships (ir), Physical Work Conditions (pc), and Structural Job Characteristics (sc). These groupings were not presented to respondents but are leveraged here to facilitate interpretation of characteristic association.} across resource, challenge, and hindrance evaluations. Here one would expect only trivial associations \emph{across} resource, challenge, and hindrance perceptions if job characteristics were uniquely categorized as such.

First, the average correlation within all resource categories (variables 1 through 7 in Table 1) was .43 (\emph{SD} = .13, coefficient magnitudes range from .15 to .64), and challenge categories exhibited similar associations (ranging from .12 to .70, \emph{M} = .43, \emph{SD} =.16). Job hindrances, however, exhibited relatively elevated associations across categories, with correlations ranging from .33 to .86, \emph{M} = .62, \emph{SD} = .17. When people perceive hindrances, these seem to be more likely to be shared across different types of job activities, whereas challenges and resources can be relatively isolated within different job activities.

Regarding comparisons \emph{across perception type}, the mean resource to challenge correlations within the same O*Net grouping ranged from .62 to .66 (\emph{M} = .64, \emph{SD} = .02; for example, the association between information input ratings as a resource and information input challenges was .62). The correlations between resources and challenges \emph{across} O*Net grouping ranged from .08 to .50, \emph{M} = .32, \emph{SD} = .12 (for example, the correlation between mental processes {[}resource{]} and work output {[}challenge{]} was .39). The resource-hindrance correlations within the same O*Net grouping ranged from -.16 to -.30 (\emph{M} = -.24, \emph{SD} = .05), while the correlations between resources and hindrances \emph{across} O*Net grouping ranged from .05 to -.27 (\emph{M} = -.14, \emph{SD} = .08). The mean challenge to hindrance correlations within the same O*Net grouping ranged from -.04 to -.27 (\emph{M} = -.21, \emph{SD} = .08). The correlations between challenges to hindrances across O*Net grouping ranged from .12 to -.26, \emph{M} = -.11, \emph{SD} = .09. In summary, observed correlations were larger \emph{within} O*Net grouping than \emph{across} (for all comparisons). Challenge and resource demands demonstrated the largest associations, with even the magnitude of across--category associations (.32) exceeding all hindrance perception associations -.11 to -.24). These results provide support for H2 -- particularly regarding \emph{resource} and \emph{challenge} perceptions. Stated differently, job characteristics should not be expected to be uniquely categorized as resource or demand.

\subsubsection{Outcome Associations}\label{outcome-associations}
...
\caption{\label{tab:unnamed-chunk-1}Outcome variable bivariate correlations.}
...
Table 2 presents empirical associations between estimates of resource, challenge, and hindrance and worker \emph{engagement}, \emph{stress}, and \emph{burnout}. Here, workers' perceptions were aggregated across all characteristics to provide indications of ``overall'' resource and demand experience. Engagement exhibited non-trivial associations with all three types of perception whereas stress was only associated with the hindrance aggregate. The more percieved hindrances, the higher the rating of stress (\(r\)=.11, \(p\)\textless.01), indicating only weak partial support for Hypothesis 3b. All subsequent analyses retain this ``overall'' operationalization of workers' experience of resources, challenges, and hindrances.

\subsubsection{Challenges, Resources, and Job Outcomes}\label{challenges-resources-and-job-outcomes}

Hypotheses 3a, 4a, and 4b were formally tested via targeted hierarchical regressions. H3a predicted that both resources and challenges would predict engagement. Table \ref{tab:chal-resource-table} summarizes the results for the hypothesized engagement effect as well as the H4a investigations of both stress and burnout. As expected given the Table 2 bivariate associations, the combined effect of challenges and resources did explain a statistically significant amount of the variability in engagement, \(R^2\) = 0.15, Adj. \(R^2\) = 0.15, \(F(2, 565) = 50.09\), \(p < .001\). However, the resource slope is significant, wheras the challenge slope is not significant. This results in only partial support for H3a. The inclusion of the interaction term in step two of the model was not hypothesized but is included for completeness. The interaction effect did contribute a significant addition to the model, \(F(3, 564) = 35.62\), \(p < .001\), \(\Delta R^2\) = 0.01, \(\Delta F\) (1, 564) = 5.82, and thus provides some statistical support for the possibility of moderation. Figure \ref{fig:chal-resource-int} illustrates the nature of the interaction. With low levels of resources, the relationship between challenges and engagement is relatively flat and engagement is comparatively low. With more resources, the relationship between challenges and engagement is negative, but engagement still remains higher with greater reported challenge when more resources are perceived.

Regarding Hypothesis 4a, challenge demands and resources did not explain a significant amount of the variance in stress, \(R^2\) = 0.01, Adj. \(R^2\) = 0.00, \(F(2, 565) = 1.67\), \(p = .189\). The inclusion of the interaction term in step two of the model did not contribute a significant addition to the model, \(F(3, 564) = 1.17\), \(p = .320\), \(\Delta R^2\) = 0.00, \(\Delta F\) (1, 564) = 0.17, and thus does not support the presence of moderation. These challenge demands and resources did, however, explain a statistically significant amount of the variability in burnout, \(R^2\) = 0.04, Adj. \(R^2\) = 0.04, \(F(2, 565) = 1.67\), \(p = .189\). The inclusion of the interaction term in step two of the model did not contribute a significant addition to the model, \(F(3, 564) = 1.17\), \(p = .320\), \(\Delta R^2\) = 0.00, \(\Delta F\) (1, 564) = 2.25, and thus collectively these analyses fail to provide statistical support for the presence of moderation (Hypothesis 4a). In sum, these findings do not provide support for the assertion that resources would moderate the relationships between challenge demands and the outcomes of strain and burnout.
...
\bibitem[\citeproctext]{ref-burr2019third}
Burr, H., Berthelsen, H., Moncada, S., Nübling, M., Dupret, E., Demiral, Y., Oudyk, J., Kristensen, T. S., Llorens, C., Navarro, A., et al. (2019). The third version of the copenhagen psychosocial questionnaire. \emph{Safety and Health at Work}, \emph{10}(4), 482--503.

\bibitem[\citeproctext]{ref-campbell1959convergent}
...
Cronbach, L. J. (1951). Coefficient alpha and the internal structure of tests. \emph{Psychometrika}, \emph{16}(3), 297--334.
