...
```{r workcat, fig.cap="Percent agreement (resource and challenge) across O*Net occupational categories (probably need to revisit original coding).", fig.height=10}
...
dataworker <- cbind(anova,workers)    ## to repeat O*Net analyses
                                     ## This gets worker info into convergence file

dataworker$overall <- rowMeans(dataworker[1:84], na.rm=TRUE)

ggplot(dataworker, aes(x=overall)) + geom_histogram() + facet_wrap(~O.Net.Job.Family.Categories, nrow=8)
